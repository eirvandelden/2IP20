...
uses
  ...
  BoundedStack, BoundedQueue, ExtCtrls, StdCtrls, Buttons;
...
type
  ...
  public
    { public declarations }
    FStack: TBoundedStackOfInt;
    FQueue: TBoundedQueueOfInt;
...
begin
  ...
  QueueMemo.Lines.Add('Tail/Head: ' + IntToStr(FQueue.First));
  ClearQueueButton.Enabled := true;
  RandomQueueButton.Enabled := true;
  RemFirstQueueButton.Enabled := not FQueue.IsEmpty;
  PutQueueButton.Enabled := (MaskEdit2.Text <> '  ') and not FQueue.IsFull;
end;
...
begin
  ...
  PushStackButton.Enabled := (MaskEdit1.Text <> '  ') and not FStack.IsFull;
end;

procedure TForm1.FormCreate(Sender: TObject);
begin
  Randomize;
  FStack := TBoundedStackOfInt.Create(coStackSize);
  RandomStack;
  UpdateStackViews;
  FQueue := TBoundedQueueOfInt.Create(coQueueSize);
  RandomQueue;
  UpdateQueueViews;
end;
