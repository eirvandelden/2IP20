...
begin
    with ANode^ do begin
        // recursively calculate childeren position
        if FRight <> nil then AuxCalc(FRight);
        if FLeft <> nil then AuxCalc(FLeft);

        // when this node is a parent (or empty subtrees need to be drawn)
        if (TreeWidth(FRight) > 0) or (TreeWidth(FLeft) > 0) then begin
            { set dimensions }
            FWidth := TreeWidth(FRight) + HSpace + TreeWidth(FLeft);
            FHeight := Max(TreeHeight(FLeft), TreeHeight(FRight)) + VSpace + 2*NodeSize;
            { set root x,y-position }
            FY := NodeSize;
            if FRootPositionStyle = 0 then
                FX := FWidth div 2
            else if FRootPositionStyle = 1 then
                FX := (TreeRootX(FLeft) + HSpace + TreeWidth(FLeft) + TreeRootX(FRight)) div 2
            else if FRootPositionStyle = 2 then
                FX := TreeWidth(FLeft) + (HSpace div 2);

        // when this node is not a parent and subtrees need not to be drawn
        end else begin
            { set dimentions }
            FWidth := 2*NodeSize;
            FHeight := 2*NodeSize;
