...
// AuxCalc opnieuw geschreven, zodanig dat er nu zonder extra functies en booleans rekening gehouden wordt
//  met de "DisplayEmptyTrees" optie. De code is stukken eenvoudigere geworden, omdat er op de juiste plaatsen
//  geen verschil gemaakt wordt tussen linker en rechter nodes.  
...
begin
  ...
  // bereken alle linker kinderen
  if ( ANode.FLeft <> nil ) then begin
    AuxCalc( ANode.FLeft );
  end;

  // bereken alle rechter kinderen
  if ( ANode.FRight <> nil ) then begin
    AuxCalc( ANode.FRight );
  end;
  
      
  // We zijn nu bij een node aangekomen. Er zijn 2 opties, dit is een laatste kind (of er is een lege node nodig)
  //  óf we zijn bij een ouder. 
  
  // Optie 0: We zijn bij een ouder / er moeten lege nodes getekent worden
  if ( TreeWidth( ANode.FRight ) > 0) or (TreeWidth( ANode.FLeft ) > 0 ) then begin     // aan een van beide kanten
    
    ANode^.FWidth := TreeWidth( ANode.FRight ) + HSpace + TreeWidth( ANode.FLeft );   //de breedte van de node
    ANode^.FHeight := Max( TreeHeight( ANode.FLeft ), TreeHeight( ANode.FRight ) ) + VSpace + ( 2 * NodeSize ); //de hoogte van de node
    ANode^.FY := NodeSize;                                  // De y-positie van de Node
    
    // De x-positie van de Node is afhankelijk van positie stijl        
    case FRootPositionStyle of                              
      0: begin
          ANode^.FX := ( ANode.FWidth ) div 2;
         end; // end case 0

      1: begin
          ANode^.FX := (TreeRootX( ANode^.FLeft ) + HSpace + TreeWidth( ANode^.FLeft ) + TreeRootX( ANode^.FRight ) ) div 2;
         end; // end case 1

      2: begin
          ANode^.FX := TreeWidth( ANode^.FLeft ) + ( HSpace div 2 );
         end; // end case 2
    end;

  end //if
  
  // Optie 1: Dit is geen ouder (of er is geen lege node nodig), omdat dit de enige ander opties is, mag dit in de else
  else begin
    ANode^.FWidth  := 2 * NodeSize;                            // De breedte van de Node
    ANode^.FHeight := 2 * NodeSize;                           // De hoogte van de Node
          
    ANode^.FY := NodeSize;                                  // De y-positie van de Node
    ANode^.FX := NodeSize;                                  // De x-positie van de Node
  end; // end else
...
begin
  ...
  with ACanvas, ANode^ do begin
    ...
    else if FDrawEmptyTrees then begin
    ...
    pen.Color     := clblack;                               // maak een zwart iets
    brush.Color   := clyellow;                              //maak een geel iets

    textRect( ARect, ARect.Left + 3, ARect.Top + 3, IntToStr( ANode.FValue ) ); //schrijf text erop
    brush.Color := clBlack;
    FrameRect( ARect );
    end;
  end;
