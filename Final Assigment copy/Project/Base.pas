...
//we maken gebruik van APlacement
with APlacement do begin

// ken geheugen toe
setLength(Result, FOrientation.PositionCount);

// bereken de posities
// sla deze op in Result

for i := 0 to FOrientation.PositionCount - 1 do begin


 // aPosition := APlacement.FOrientation.Position(i);
  Result[i].Row := FOrientation.Position(i).Row + FPosition.Row;	
  Result[i].Col := FOrientation.Position(i).Col + FPosition.Col;  
  end; //for
...
//# BEGIN TODO TBox.AreCellsFree implementation statements
 ...
 // (forall (Row, Col) in CoverP(APlacement) :
 //         ValidCoordinates(Row, Col) and CellState(Row, Col) = csFree)
...
// init
 ...
 while Result = True and (i <> APlacement.FOrientation.PositionCount {-1}) do begin
 with VPositionlist[i] do begin
   Result := ValidCoordinates(Row, Col) and (CellState(Row, Col) = csFree);
   inc(i);
 end; //whith
 end; //while
